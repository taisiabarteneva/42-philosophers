...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/04 21:42:46 by wurrigon          #+#    #+#             */
/*   Updated: 2022/02/04 23:50:36 by wurrigon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static void	dine(t_philosopher *philo, t_mutex *min, t_mutex *max, long time)
{
...
static void	cycle(t_philosopher *philo, t_mutex *min, t_mutex *max, long time)
{
	while (1)
	{
...
void	*lifetime(void *thread)
{
	...
	if (philo->num % 2 == 0)
		usleep(100);
	time = philo->constants->program_start.tv_sec * \
		1000 + philo->constants->program_start.tv_usec / 1000;
...
int	init_threads(t_data *data)
{
	...
	{
		if (pthread_create(&data->philos[i].t_id, NULL, &lifetime,
				(void *)&data->philos[i]) != SUCCESS)
			return (FAILURE);
			i++;
	}
	return (SUCCESS);
